...
### 11.5 1단계와 2단계는 담당자가 직접 만든다

밤에는 두 단계의 골격 코드까지 만들어 두었는데, 아침에 대표가 되돌렸다. 이유는 분명하다. 팀원 파트를 대신 구현해 주면 그 사람은 자기 코드를 이해하지 못한 채 떠안게 되고, 과제에서 배우는 것이 없어진다. 그래서 코드는 참고용 브랜치(`reference/claude-v0`)로 치우고, 저장소에는 **설계 지침**만 남겼다.
...
담당자가 바로 시작할 수 있게 지루한 조사는 밤에 끝냈다. `docs/research/`의 보고서 네 편이 그것이다. 정적 규칙의 출처와 라이선스(R1), 기존 LLM 판정기들의 프롬프트 설계와 현행 API 가격(R2), 기성 스캐너 넷의 설치·실행 방법(R3), 논문들의 평가 규약(R4). 모든 수치에 원문 URL이 달려 있다.
...
- **1단계**: `docs/stage1-guide.md`를 읽고 규칙 출처 중 어느 것을 쓸지 정한 뒤, 스킬 폴더 하나를 읽어 점수 하나를 내는 가장 작은 스캐너를 직접 짠다. dev 분할 50건에 돌려 보고 결과를 실험 노트에 붙인다
- **2단계**: `docs/stage2-guide.md`를 읽고 SkillGate 공개 프롬프트 원문을 직접 본 뒤, 자기 프롬프트 v1을 쓴다. API 키와 예산을 확인하고 dev 20건에 돌려 JSON이 제대로 나오는지 본다

### 11.10 첫 숫자, 그리고 정직하게 적어 둘 것

참고 삼아 Claude가 만든 v0 스캐너(공개 규칙 59개를 모은 것, `reference/claude-v0` 브랜치)를 dev 분할 6,674건에 돌려 봤다. 우리 탐지기의 성적이 아니라 데이터가 어떤 모양인지 보는 용도다.
...
이 밤에 한 일은 "연구"가 아니라 "착수 준비"다. 벤치마크를 읽고 정리하고, 규칙과 프롬프트의 출처를 조사하고, 파일 규약을 정하고, 데이터가 어떤 모양인지 숫자 하나로 확인한 것이다. 탐지기를 만드는 것은 지금부터이고, 그것은 각자의 손으로 한다.
